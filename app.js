...
function deploy(user) {
   ...
   // Construct the deploy request
    ...
    console.log("deploy %s for user \n", user);

    console.log("deploy %s for user \n", user.name);

    console.log("deploy %s for user \n", user.enrollment.cert);


    var userObject = new Object();
    userObject.name = user.name;
    userObject.cert  = user.enrollment.cert;
    
   var deployRequest = {
       chaincodeName: process.env.CORE_CHAINCODE_ID_NAME,
       fcn: "init",
       args: ["a", "10", "b", "20", JSON.stringify(userObject) ]
   };
   ...
   //deployRequest.chaincodePath = "github.com/github.com/branciard/paza/tree/master/chaincode";
